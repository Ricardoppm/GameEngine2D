...
namespace Bengine {
    ...
    {
        // Check if renderer and system are already initialized
    ...
    void GUI::destroy()
    {
        ...
        CEGUI::WindowManager::getSingleton().destroyWindow(root_);
        context_ = nullptr;
        root_ = nullptr;
    }
    ...
    void GUI::update()
    {
        ...
        context_->injectTimePulse( (float)elapsed / 1000.f );
    }
    
    void GUI::setMouseCursor(const std::string& mouseFile)
    {
        context_->getMouseCursor().setDefaultImage(mouseFile);
    }
    
    void GUI::showMouseCursor() { context_->getMouseCursor().show(); }
    
    void GUI::hideMouseCursor(){ context_->getMouseCursor().hide(); }
    ...
    void GUI::onSDLEvent(SDL_Event &e)
    {
        CEGUI::String cs;
        switch (e.type) {
            case SDL_MOUSEMOTION:
                context_->injectMousePosition((float)e.motion.x, (float)e.motion.y);
                break;
                
            case SDL_KEYDOWN:
                context_->injectKeyDown(SDLKeyToCEGUIKey(e.key.keysym.sym));
                break;
                
            case SDL_KEYUP:
                context_->injectKeyUp(SDLKeyToCEGUIKey(e.key.keysym.sym));
                break;
            
            case SDL_TEXTINPUT:
                cs = CEGUI::String((CEGUI::utf8*) e.text.text);
                for(int i = 0; i < cs.size(); ++i){
                    context_->injectChar(cs[i]);
                }
                break;
            
            case SDL_MOUSEBUTTONDOWN:
                context_->injectMouseButtonDown(SDLButtonToCEGUIButton(e.button.button));
                break;
            
            case SDL_MOUSEBUTTONUP:
                context_->injectMouseButtonUp(SDLButtonToCEGUIButton(e.button.button));
                break;
                           ...
                           const std::string& name )
    {
        ...
        setWidgetDestRect(newWindow, destRectPerct, destRectPix);
        return newWindow;
    }
    ...
    void GUI::setWidgetDestRect(CEGUI::Window *widget,
                                ...
                                const glm::vec4 &destRectPix)
        {
        widget->setPosition(CEGUI::UVector2(CEGUI::UDim(destRectPerct.x, destRectPix.x),
                                            CEGUI::UDim(destRectPerct.y, destRectPix.y)));
        widget->setSize(CEGUI::USize(CEGUI::UDim(destRectPerct.z, destRectPix.z),
                                     CEGUI::UDim(destRectPerct.w, destRectPix.w)));
    }
